...
# %% [markdown]
# La conversion en tableau numpy a homogénéisé le type des valeurs.
...
print(df.loc[2,'lifespan'], df.names.dtype)

# %% [markdown]
# La valeur (numérique) du lifespan de l'elephant a été convertie en un type plus général (object) lors de la création du tableau numpy.

# %%
# 5

print(df.loc[2,'names'], df.names.dtype)

# %% [markdown]
# Le type est le même que les éléments de la colonne lifespan, cela est cohérent avec le fait que numpy a homogénéisé les valeurs lors de la création du tableau
...
# %% [markdown]
# En introduisant un élément de type string dans une colonne de type object, pandas a automatiquement converti cet élément en object afin de garder une colonne homogène avec un type plus général.
...
# %% [markdown]
# Le type du tableau est encore `object`...
...
# %% [markdown]
# Numpy a pris le type le plus général (ici object) pour rassembler les trois colonnes dans un même tableau. On n'a plus comme au début un tableau de type Unicode32.
...
# %% scrolled=true
# ici, sans préciser de paramètres,
...
# %% cell_style="split" tags=["level_intermediate"] scrolled=true
...
# %% tags=["level_intermediate"]
pd.concat( [ pd.merge( df1i, df2i, left_index = True, right_index = True) , df3i])
