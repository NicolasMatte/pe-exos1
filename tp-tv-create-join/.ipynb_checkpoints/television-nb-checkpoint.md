...
def is_empty_column(df, colname):
    """True si et seulement si colname est remplie de vide"""
    return pd.notna(df[colname]).sum() == 0
```
...
col5 = 'Unnamed: 4'
print(is_empty_column(df, col5))
```

```{code-cell} ipython3
:scrolled: true

is_empty_column(df, df.columns)
```
...
df.drop( df.columns[is_empty_column(df, df.columns)], axis=1, inplace=True )
df
```
...
Le test d'égalité entre deux objets nan renvoie toujours False.
...
```{code-cell} ipython3
# à vous
nb_lines_to_clean = pd.isna(df.cLT2FREQ).sum()
```
...
df.drop( df[pd.isna(df.cLT2FREQ)].index, inplace=True )
```
